    ...
    <button class="contribs-button" data-carousel-button="contribs" on:click={switch_slide}>
        ...
        <table>
            {#each range(0, num_rows) as i}
                {#each range(0, num_cols) as j}
                    <td style:grid-column={`${j + 1} / ${j + 2}`} style:grid-row={`${i + 1} / ${i + 2}`} style:--btn-grid-animation-order={j + i}></td>
                {/each}
            {/each}
        </table>
    </button>
    <a class="cv-button" href="https://raw.githubusercontent.com/08Aristodemus24/auto-job-app-sender/master/documents/tailored/updated/data_engineer_cv_wsum_full_template_new.pdf" download="Larry_Miguel_R_Cueva_CV.pdf">
